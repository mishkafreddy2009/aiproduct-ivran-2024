...
""" Пасер, перевод текста """

class Parser():
    def __int__(self):
        self.WARCS_PATH = "./dataset/ja/"
        self.YA_TRANSLATE_TOKEN = os.environ["YA_TRANSLATE_TOKEN"]
    def call_api(self, url: str, data: dict) -> dict:
        headers = { "Authorization" : f"Api-Key {self.YA_TRANSLATE_TOKEN}" }
        response = requests.post(url, json=data, headers=headers)
        return response.json()

    def extract_content(self, record: ArcWarcRecord) -> str | None:
    ...
    def main(self):
        with open(self.WARCS_PATH + "ascii.jp-2016-12.warc", "rb") as stream:
            for record in ArchiveIterator(stream):
                content = self.extract_content(record)
                if content:
                    data = self.parse_content(content)
                    print(self.translate_text(data))
                    break
    #
